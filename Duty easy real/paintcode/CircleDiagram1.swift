...
class CircleDiagram1: NSObject {
    ...
    class func drawUntitled(frame targetFrame: CGRect = CGRect(x: 0, y: 0, width: 200, height: 204), resizing: ResizingBehavior = .aspectFit) {
        ...
        do {
            ...
            /// Circle 1
            let circle1 = NSMutableAttributedString(string: "Circle 1")
            circle1.addAttribute(.font, value: UIFont(name: "Helvetica", size: 18)!, range: NSRange(location: 0, length: circle1.length))
            do {
                let paragraphStyle = NSMutableParagraphStyle()
                paragraphStyle.alignment = .center
                circle1.addAttribute(.paragraphStyle, value: paragraphStyle, range: NSRange(location: 0, length: circle1.length))
            }
            context.saveGState()
            circle1.draw(at: CGPoint(x: 69, y: 89))
